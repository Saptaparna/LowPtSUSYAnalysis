...
void Optimization2D_h_HT_Memu_LowCutValue(std::string outfile){
  ...
  double signal_stop_SF_180 = (1.66*0.03592*1000*7.4*0.33*0.33*2)/99986;
  ...
  TFile* file10 = TFile::Open("MC_OS_Signal_OptimizationStudies/Output_LowPtSUSY_Tree_Stop180_Chargino155_Neutralino110_Ntuple_Pta20.root");
  ...
  for (int j=1; j<=yaxis->GetNbins();j++) {
      for (int i=1; i<=xaxis->GetNbins();i++) {
         ...
         if(sumOfBackgrounds > 0.0) h_Significance->SetBinContent(i, j, (h10->GetBinContent(i, j)*signal_stop_SF_180/
