...
#include <TH2.h>
#include <TStyle.h>
#include "/uscms_data/d1/sapta/work/SUSYSearch/CMSSW_5_3_11_patch6/src/MPAnalyzer/PhysicsTools/Utilities/interface/LumiReweightingStandAlone.h"
...
void hg_selector::Begin(TTree *tree)
{
   ...
   /*
   tm->AddVar("phiso","float");
   tm->AddVar("niso","float");
   tm->AddVar("chiso","float");
   tm->AddVar("track03iso","float");
   tm->AddVar("hadron03iso","float");
   tm->AddVar("ecal03iso","float");
   tm->AddVar("track04iso","float");
   tm->AddVar("hadron04iso","float");
   tm->AddVar("ecal04iso","float");
   */
   tm->AddVar("run","int");
   tm->AddVar("lumi","int");
   tm->AddVar("event","int");

   tm->AddVar("cscTightHalo","bool");
   ...
   tm->AddVar("Pho_Energy","float");
   tm->AddVar("Pho_Eta_Old", "float");
   ...
   tm->AddVar("Preselection","int");
   tm->AddVar("n_jets_mva_loose", "int");
   tm->AddVar("n_jets_mva_medium", "int");
   tm->AddVar("n_jets_mva_tight", "int");
   tm->AddVar("n_jets_cut_loose", "int");
   tm->AddVar("n_jets_cut_medium", "int");
   tm->AddVar("n_jets_cut_tight", "int");
   ...
   if(mc) tm->AddVar("sum_genPt","float");
   ...
   tm->AddVar("fired_HLTPhoIdMet", "bool");
   tm->AddVar("fired_HLTPhoId", "bool");
   tm->AddVar("fired_HLTPho", "bool");
   
   tm->AddVar("LICTD","float");

   tm->AddVar("foundisoel","int");
   tm->AddVar("foundlooseEl","int");
   tm->AddVar("foundvetoEl","int");
   tm->AddVar("foundisomu","int");
   tm->AddVar("loose_mu","int");
   //tm->AddVar("mu_dz","float");
   //tm->AddVar("mu_dxy","float");
   //tm->AddVar("mu_pt","float");
   //tm->AddVar("mu_eta","float");
   //tm->AddVar("el_pt","float");
   //tm->AddVar("el_eta","float");

   tm->AddVar("el_pt_leading","float");
   tm->AddVar("el_phi_leading","float");
   tm->AddVar("el_eta_leading","float");
   tm->AddVar("el_energy_leading","float");
   tm->AddVar("el_charge_leading","float");
   tm->AddVar("el_pt_trailing","float");
   tm->AddVar("el_phi_trailing","float");
   tm->AddVar("el_eta_trailing","float");
   tm->AddVar("el_energy_trailing","float");
   tm->AddVar("el_charge_trailing","float");
   tm->AddVar("mu_pt_leading","float");
   tm->AddVar("mu_phi_leading","float");
   tm->AddVar("mu_eta_leading","float");
   tm->AddVar("mu_energy_leading","float"); 
   tm->AddVar("mu_charge_leading","float");
   tm->AddVar("mu_pt_trailing","float");
   tm->AddVar("mu_phi_trailing","float");
   tm->AddVar("mu_eta_trailing","float");
   tm->AddVar("mu_energy_trailing","float"); 
   tm->AddVar("mu_charge_trailing","float");
   ...
   tm->AddVar("ph_pt_leading", "float");
   tm->AddVar("ph_phi_leading", "float");
   tm->AddVar("ph_eta_leading", "float");
   tm->AddVar("ph_energy_leading", "float");
   tm->AddVar("ph_pt_trailing", "float");
   tm->AddVar("ph_phi_trailing", "float");
   tm->AddVar("ph_eta_trailing", "float");
   tm->AddVar("ph_energy_trailing", "float");
...
Bool_t hg_selector::Process(Long64_t entry)
{
  ...
  if(!mc){
    ...
    if (NoiseFilters_isCSCTightHalo) {if(verbose) cout<< "NoiseFilters_isCSCTightHalo: " << NoiseFilters_isCSCTightHalo <<endl; tm->SetValue("cscTightHalo",NoiseFilters_isCSCTightHalo); return kTRUE;}
  ...
  *pvPosition = goodVertices[0];
  ...
  for (Int_t i = 0; i < recoPhotons->GetSize(); ++i) {
    ...
    float chEA,nhEA,phEA;

    float tmpEta = thisPhoton->SCEta();
    ...
    if(   (thisPhoton->IsoMap("EmIso_R03")  - 0.012*thisPhoton->Pt()) > 4
       || (thisPhoton->IsoMap("HadIso_R03") - 0.005*thisPhoton->Pt()) > 4
       || (thisPhoton->IsoMap("TrkIso_R03") - 0.002*thisPhoton->Pt()) > 4
       || thisPhoton->R9()<0.9
       || thisPhoton->HadOverEm() > 0.10
       || thisPhoton->SigmaIEtaIEta() > 0.024
	  ) preselection = 0;
      
    
    if(!qcd) {
      if (!PassPhotonID(thisPhoton, mediumPhID)) continue;
      if (!PassPhotonIso(thisPhoton, mediumPhIso)) continue;
    }
    if(qcd) {
      if (!PassNewQCDID(thisPhoton)) continue;
    }
    //if (!(thisPhoton->Pt()>30.)) continue;
    if (!(fabs(thisPhoton->SCEta())<1.4442)) continue;
    if (!(thisPhoton->R9()>0.9)) continue;
    ...
    if(NumGoodPhotons == 1 ) {      
      ...
      tm->SetValue("Pho_Energy",thisPhoton->Energy());
      tm->SetValue("Pho_Eta_Old",thisPhoton->Eta());
      tm->SetValue("Pho_R9",thisPhoton->R9());
    ...
    tm->SetValue("ph_pt_leading", photonsIDIso.size() < 1 ? -99 : photonsIDIso[0].Pt());
    tm->SetValue("ph_phi_leading",photonsIDIso.size() < 1 ? -99 : photonsIDIso[0].Phi());
    tm->SetValue("ph_eta_leading",photonsIDIso.size() < 1 ? -99 : photonsIDIso[0].Eta());
    tm->SetValue("ph_energy_leading",photonsIDIso.size() < 1 ? -99 : photonsIDIso[0].Energy());
    tm->SetValue("ph_pt_trailing",photonsIDIso.size() > 1 ? photonsIDIso[1].Pt() : -99);
    tm->SetValue("ph_phi_trailing",photonsIDIso.size() > 1 ? photonsIDIso[1].Phi() : -99);
    tm->SetValue("ph_eta_trailing",photonsIDIso.size() > 1 ? photonsIDIso[1].Eta() : -99);
    tm->SetValue("ph_energy_trailing",photonsIDIso.size() > 1 ? photonsIDIso[1].Energy() : -99);
  ...
  if(mc){
    ...
    for(int i = 0; i <  triggerObjects->GetSize(); ++i) {
    TCTriggerObject* thisTrigObj = (TCTriggerObject*) triggerObjects->At(i);
    //cout << "Triggers in MC = " << thisTrigObj->GetHLTName() << endl;
     }
 ...
 /*   for (int i = 0; i <  triggerObjects->GetSize(); ++i) {
  ...
  tm->SetValue("fired_HLTPho", fired_HLTPho);
  tm->SetValue("fired_HLTPhoId", fired_HLTPhoId);
  tm->SetValue("fired_HLTPhoIdMet", fired_HLTPhoIdMet); 
  
  ///////////////
  // MET Stuff //
  ///////////////
  
  //if(corrMET->Mod() < 40.) return kTRUE;
  float MEx_corr;
  float MEy_corr;

  if(!mc){
    //data
    MEx_corr = corrMET->Px() - (0.639521 + 0.229545*(nvtx));
    MEy_corr = corrMET->Py() - (-0.479465 - 0.0647124*(nvtx));
  }
  else{
    if(pjets){
      //p+jets                                                 
      MEx_corr = corrMET->Px() - (0.186578 - 0.0323318*nvtx);
      MEy_corr = corrMET->Py() - (-0.277675 - 0.0873093*nvtx);
    }
    else{
      //znunuG
      MEx_corr = corrMET->Px() - (0.0626902 - 0.0141362*(nvtx));
      MEy_corr = corrMET->Py() - (-0.146358 - 0.0830809*(nvtx));
    }
  }
  ...
  tm->SetValue("MT_old",TransverseMass(Photon.Pt(), Photon.Px(), Photon.Py(), corrMET->Mod(), corrMET->Px(), corrMET->Py()));
  tm->SetValue("MT",TransverseMass(Photon.Pt(), Photon.Px(), Photon.Py(), Met_corr, MEx_corr,MEy_corr));
	

  //if(Met_corr <= 40.) return kTRUE;
  ...
  // Jet Veto  //
  ///////////////
  int count_jets_mva_loose = 0; int count_jets_mva_medium = 0; int count_jets_mva_tight = 0; int count_jets_cut_loose = 0; int count_jets_cut_medium = 0; int count_jets_cut_tight= 0;
  ...
  for(int jet = 0; jet < patJets->GetSize(); jet++){
	...
	if(count_jets_mva_loose == 1) { 
      ...
      if(Jet->PuJetIdFlag_mva_medium()==1)  count_jets_mva_medium++;
      if(Jet->PuJetIdFlag_mva_tight()==1)  count_jets_mva_tight++;
      
      if(Jet->PuJetIdFlag_cut_loose()==1)  count_jets_cut_loose++;
      if(Jet->PuJetIdFlag_cut_medium()==1)  count_jets_cut_medium++;
      if(Jet->PuJetIdFlag_cut_tight()==1)  count_jets_cut_tight++;
    }
  }
  tm->SetValue("n_jets_mva_loose", count_jets_mva_loose);
  tm->SetValue("n_jets_mva_medium", count_jets_mva_medium);
  tm->SetValue("n_jets_mva_tight", count_jets_mva_tight);
  tm->SetValue("n_jets_cut_loose", count_jets_cut_loose);
  tm->SetValue("n_jets_cut_medium", count_jets_cut_medium);
  tm->SetValue("n_jets_cut_tight", count_jets_cut_tight);
  ...
  if (verbose) cout << "STPrime MET = " << STPrime << endl;
  ...
  if (verbose) cout << "STPrime Photons = " << STPrime << endl;
  ...
  for (int i = 0; i <  recoElectrons->GetSize(); ++i) {
    ...
    hitmap = thisElec->HitMap();
    for (int s = 0; s < thisElec->HitMap().size(); s++){
      //std::cout << hitmap[s].Layer << std::endl;
    }
    ...
    if (thisElec->DeltaR(Photon) > 0.3 && PassElectronID(thisElec, looseElID) && PassElectronIso(thisElec, looseElIso)) { found_looseEl = 1;}
    if (thisElec->DeltaR(Photon) > 0.3 && PassElectronID(thisElec,vetoElID)&& PassElectronIso(thisElec, looseElIso)) {found_vetoEl =1;}
  ...
  if (verbose) cout << "STPrime electrons = " << STPrime << endl; 

  if(verbose) std::cout <<"Exiting the electron loop with "<<electronIDIso.size() <<" electrons.."<< std::endl;

  tm->SetValue("minDR",mindR);

  //float pt_0  = electronIDIso.size() < 1 ? -99 : electronIDIso[0].Pt()  ;
  //std::cout << "pt_0: "<< pt_0 << std::endl;

  //if(electronIDIso.size() > 0){
    ...
    tm->SetValue("el_energy_leading",electronIDIso.size() < 1 ? -99 : electronIDIso[0].Energy());
    tm->SetValue("el_charge_leading",electronIDIso.size() < 1 ? -99 : electronIDIso[0].Charge());
    tm->SetValue("el_pt_trailing",electronIDIso.size() > 1 ? electronIDIso[1].Pt() : -99);
    tm->SetValue("el_phi_trailing",electronIDIso.size() > 1 ? electronIDIso[1].Phi() : -99);
    tm->SetValue("el_eta_trailing",electronIDIso.size() > 1 ? electronIDIso[1].Eta() : -99);
    tm->SetValue("el_energy_trailing",electronIDIso.size() > 1 ? electronIDIso[1].Energy() : -99);
    tm->SetValue("el_charge_trailing",electronIDIso.size() > 1 ? electronIDIso[1].Charge() : -99); 
   //}
  ...
  for (int i = 0; i < recoMuons->GetSize(); ++ i)  {
    ...
    //tm->SetValue("mu_dxy",thisMuon->Dxy(pvPosition));
    //tm->SetValue("mu_dz", thisMuon->Dz(pvPosition));
    //tm->SetValue("mu_pt",thisMuon->Pt());
    //tm->SetValue("mu_eta",thisMuon->Eta());
    
    if (thisMuon->DeltaR(Photon) > 0.3 && thisMuon->IsPF() == 1 && (thisMuon->IsGLB() == 1  || thisMuon->IsTRK() == 1 ) && PassMuonIso(thisMuon, looseMuIso)){ 
      found_looseMuon = 1; }
  ...
  if (verbose) cout << "STPrime muons = " << STPrime << endl;
  ...
  tm->SetValue("mu_energy_leading",muonIDIso.size() < 1 ? -99 : muonIDIso[0].Energy());
  tm->SetValue("mu_charge_leading",muonIDIso.size() < 1 ? -99 : muonIDIso[0].Charge());
  tm->SetValue("mu_pt_trailing",muonIDIso.size() > 1 ? muonIDIso[1].Pt() : -99);
  tm->SetValue("mu_phi_trailing",muonIDIso.size() > 1 ? muonIDIso[1].Phi() : -99);
  tm->SetValue("mu_eta_trailing",muonIDIso.size() > 1 ? muonIDIso[1].Eta() : -99);
  tm->SetValue("mu_energy_trailing",muonIDIso.size() > 1 ? muonIDIso[1].Energy() : -99);
  tm->SetValue("mu_charge_trailing",muonIDIso.size() > 1 ? muonIDIso[1].Charge() : -99);
  //}
  ...
  if (verbose) cout << "STPrime jets = " << STPrime << endl; 
  ...
  /*
  if(Photon.Pt() > 30.){
    std::cout <<"MET: " << corrMET->Mod() << " MT: " << TransverseMass(Photon.Pt(), Photon.Px(), Photon.Py(), corrMET->Mod(), corrMET->Px(), corrMET->Py()) << " Px: " << corrMET->Px() <<" Py: " << corrMET->Py() << std::endl;
    
    std::cout << "MinMET: " << MinMET << " MinMT: " << TransverseMass(Photon.Pt(), Photon.Px(), Photon.Py(), MinMET, -ParPx, -ParPy) << " Px: " << -ParPx << " Py: " << -ParPy << std::endl;
  }
  */
