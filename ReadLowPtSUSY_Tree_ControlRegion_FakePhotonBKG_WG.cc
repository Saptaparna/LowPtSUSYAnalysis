#include "ReadLowPtSUSY_Tree_ControlRegion_FakePhotonBKG_WG.h"

int ReadLowPtSUSY_Tree_ControlRegion_FakePhotonBKG_WG(std::string infile, std::string outfile, std::string type, std::string signSelection){
  ...
  double ee_OS = 0;
  double ee_SS = 0;
  double mumu_OS_Zwindow = 0;

  //TFile *fakerate=new  TFile("FakeRate_MediumWP.root");
  //TFile *fakerate=new  TFile("LooseWP.root");
  //TFile *fakerate=new  TFile("FittedFakeRate.root");
  //TFile *fakerate=new  TFile("FakeRate_LWP.root");
  //TFile *fakerate=new  TFile("FittedFakeRate_LWP.root"); 
  //TFile *fakerate=new  TFile("FittedFakeRate_WithSpikeCleaning.root");
  TFile *fakerate=new TFile("FakeRate_ForWolfgang.root");
  ...
  if(fabs(photons.at(k).eta)<1.444 and returnDenPhoton(photons.at(k))==true){// and photons.at(k).phSigmaIetaIeta > 0.001 and photons.at(k).phSigmaIphiIphi>0.0001 and swissCross.at(k)<0.90){// and photons.at(k).phSigmaIetaIeta > 0.001){ //and photons.at(k).phSigmaIphiIphi>0.0001){
...
// Now sorting this vector of structs
  ...
  if (not(PhotonDen_vector.size() > 0 and PhotonDen_vector.at(0).Pt() > 32.0 and caloMET > 35.0)) continue;
  ...
  if(eventWeight > 0.0 and Muons.size()==2){ 
    if(type=="MC") eventWeight *= muonSF(Muons.at(0).LepLV.Pt(), Muons.at(0).LepLV.Eta())*muonSF(Muons.at(1).LepLV.Pt(), Muons.at(1).LepLV.Eta())*photonSF(PhotonDen_vector.at(0).Pt(), PhotonDen_vector.at(0).Eta()); 
      ...
      if(signSelection=="OS" and (Muons.at(0).Charge*Muons.at(1).Charge)==-1){
        ...
        if((Muons.at(0).LepLV+Muons.at(1).LepLV+PhotonDen_vector.at(0)).M() < 80 or (Muons.at(0).LepLV+Muons.at(1).LepLV+PhotonDen_vector.at(0)).M() > 100){
        mumu_OS+=eventWeight;
        if((Muons.at(0).LepLV+Muons.at(1).LepLV).M() > 70 and (Muons.at(0).LepLV+Muons.at(1).LepLV).M() < 110) mumu_OS_Zwindow+=eventWeight;
 ...
 //ElMu channel
  ...
  //Cut flow table
  cout << "mumu_OS_Zwindow = " << mumu_OS_Zwindow <<  endl;
