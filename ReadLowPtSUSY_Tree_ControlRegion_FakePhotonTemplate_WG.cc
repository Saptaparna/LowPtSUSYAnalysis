#include "ReadLowPtSUSY_Tree_ControlRegion_FakePhotonTemplate_WG.h"

int ReadLowPtSUSY_Tree_ControlRegion_FakePhotonTemplate_WG(std::string infile,  std::string outfile,  std::string type){
  ...
  for (int i=0; i<nEvents ; ++i)
   ...
   for(unsigned int k=0; k<photons.size(); ++k)
   {
     ...
     if(fabs(photons.at(k).eta)<1.444 and photons.at(k).pT>30.0 and photons.at(k).phHoE < 0.05 and photons.at(0).phR9 > 0.9 and photons.at(k).phpixelVeto==0 and photons.at(k).nuIsolation < (3.5 + 0.04*photons.at(k).pT) and photons.at(k).phIsolation < (1.3 + 0.005*photons.at(k).pT) and photons.at(k).phSigmaIetaIeta > 0.001 and swissCross.at(k)<0.90 and std::sqrt(photons.at(k).phSigmaIphiIphi)>0.009 and photons.at(k).phR9 < 1.0)//and photons.at(k).phSigmaIetaIeta > 0.001 and swissCross.at(k)<0.90 and photons.at(k).phSigmaIphiIphi>0.0001) //photons.at(k).phSigmaIetaIeta > 0.001 and photons.at(k).phSigmaIphiIphi>0.0001) //sideband region cut without sieie and swiss cross cut and additional cuts for beam halo rejection
     {
