...
void ntupleProducer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(saveJets_){
    ...
    for (vector<pat::Jet>::const_iterator ipJet = patjets->begin(); ipJet != patjets->end(); ++ipJet) {
      ...
      TCJet* jetCon (new ((*patJets)[jetCount_pat]) TCJet);
...
*/
	...
	myPhoton->SetPreShowerOverRaw(iPhoton->superCluster()->preshowerEnergy() / iPhoton->superCluster()->rawEnergy());
      ...
      myPhoton->SetE5x1(lazyTool->e5x1(*phoSeed));
  ...
  */
  ...
  else if(skimLepton_ && muCount > 0) // possibly specify a cut in configuration
    eventTree -> Fill();
...
float ntupleProducer::MatchBTagsToJets(const reco::JetTagCollection bTags, vector<pat::Jet>::const_iterator jet)
{
  float discrValue = -999;
  for (tag_iter iTag = bTags.begin(); iTag != bTags.end(); iTag++) {
    if ((fabs(iTag->first->eta()-jet->eta())< 0.001) and (deltaPhi(iTag->first->phi(), jet->phi()) < 0.001)) {
      discrValue = iTag->second;
      break;
    }
  }
  if (discrValue==-999) {std::cout<<"WARNING: PAT jet not found in b-tag collection"<<std::endl;}
  return discrValue;
}
