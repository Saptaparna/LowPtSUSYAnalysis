...
void makeStackedHisto_h_mu_dz_trailing_MuMu(){
...
THStack hs("hs","Dz: Trailing Muon");

TFile* file0 = TFile::Open("MC_OS/Output_LowPtSUSY_Tree_ZGToLLG_8TeV_CaloMET_All.root");
TH1F *h0 = (TH1F*)file0->Get("h_mu_dz_trailing_MuMu");
h0->Rebin(500);
h0->GetXaxis()->SetRangeUser(-0.1, 0.1);
...
TH1F *hSS = (TH1F*)fileSS->Get("h_mu_dz_trailing_MuMu");
hSS->Rebin(500);
hSS->GetXaxis()->SetRangeUser(-0.1, 0.1);
hSS->SetFillColor(kRed+3);
hs.Add(hSS);

TFile* file1 = TFile::Open("MC_OS/Output_LowPtSUSY_Tree_ZGToLLG_8TeV_CaloMET_MuMu.root");
TH1F *h1 = (TH1F*)file1->Get("h_mu_dz_trailing_MuMu");
h1->Rebin(500);
...
TH1F *h1a = (TH1F*)file1a->Get("h_mu_dz_trailing_MuMu");
h1a->Rebin(500);
h1a->GetXaxis()->SetRangeUser(-0.1, 0.1);
...
TH1F *h2 = (TH1F*)file2->Get("h_mu_dz_trailing_MuMu");
h2->Rebin(500);
...
TH1F *h6 = (TH1F*)file6->Get("h_mu_dz_trailing_MuMu");
h6->Rebin(500);
h6->GetXaxis()->SetRangeUser(-0.1, 0.1);
h6->Scale(Tbar_tW_SF);
h6->SetFillColor(kGray);
hs.Add(h6);

TFile* file8 = new TFile("MC_OS/Output_LowPtSUSY_Tree_WZJetsTo3LNu_CaloMET_All.root");
TH1F *h8 = (TH1F*)file8->Get("h_mu_dz_trailing_MuMu");
h8->Rebin(500);
h8->GetXaxis()->SetRangeUser(-0.1, 0.1);
h8->Scale(WZ_SF);
h8->SetFillColor(kViolet+1);
hs.Add(h8);

TFile* file9 = new TFile("MC_OS/Output_LowPtSUSY_Tree_WWGJets_8TeV_madgraph_CaloMET_All.root");
TH1F *h9 = (TH1F*)file9->Get("h_mu_dz_trailing_MuMu");
h9->Rebin(500);
h9->GetXaxis()->SetRangeUser(-0.1, 0.1);
h9->Scale(WWG_SF);
...
hs.SetMinimum(0.1);
hs.GetXaxis()->SetRangeUser(-0.1, 0.1);
hs.GetXaxis()->SetTitle("Dz: Trailing Muon (cm)");
...
TH1F *h_Data = (TH1F*)file_Data->Get("h_mu_dz_trailing_MuMu");
h_Data->Rebin(500);
h_Data->SetLineColor(kBlack);
h_Data->SetMarkerStyle(20);
h_Data->SetMarkerSize(1.0);
h_Data->GetXaxis()->SetRangeUser(-0.1, 0.1);
h_Data->Draw("SAME E");

TFile* file_Signal = TFile::Open("Signal/Output_LowPtSUSY_Tree_Stop100_Chargino75_Neutralino30_Ntuple_Pta20.root");
TH1F *h_Signal = (TH1F*)file_Signal->Get("h_mu_dz_trailing_MuMu");
h_Signal->Rebin(500);
h_Signal->GetXaxis()->SetRangeUser(-0.1, 0.1);
...
TH1F *h_Signal_Stop = (TH1F*)file_Signal->Get("h_mu_dz_trailing_MuMu");
h_Signal_Stop->Rebin(500);
h_Signal_Stop->GetXaxis()->SetRangeUser(-0.1, 0.1);
...
h_ratio->SetTitle("; Dz: Trailing Muon (cm); Data/MC Ratio");
...
h_ratio->GetXaxis()->SetRangeUser(-0.1, 0.1); 
h_ratio->Draw();

TF1 *fit_ratio = new TF1("fit_ratio","[0]*x + [1]",-0.1, 0.1);
fit_ratio->SetParLimits(0,0.0,0.001);
fit_ratio->SetParLimits(1,0.0, 1.2);
fit_ratio->SetLineColor(kRed);
fit_ratio->SetLineWidth(3);
h_ratio->Fit("fit_ratio", "", "", -0.1, 0.1);
...
c1.SaveAs("h_mu_dz_trailing_MuMu_Stacked.pdf");
c1.SaveAs("h_mu_dz_trailing_MuMu_Stacked.png");
}
