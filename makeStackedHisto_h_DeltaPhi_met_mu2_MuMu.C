...
THStack hs("hs","#Delta#phi(MET, trailing muon)");
...
TH1F *hSS = (TH1F*)fileSS->Get("h_DeltaPhi_met_mu2_MuMu");
hSS->Rebin(100);
hSS->GetXaxis()->SetRangeUser(-4, 4);
...
h9->Rebin(100);
h9->GetXaxis()->SetRangeUser(-4, 4);
h9->Scale(WWG_SF);
h9->SetFillColor(kMagenta);
hs.Add(h9);

hs.Draw("HIST");
hs.SetMaximum(40000.0);
hs.SetMinimum(0.1);
...
h_Signal->Rebin(100);
h_Signal->GetXaxis()->SetRangeUser(-4, 4);
...
TH1F *h_Signal_Stop = (TH1F*)file_Signal->Get("h_DeltaPhi_met_mu2_MuMu");
h_Signal_Stop->Rebin(100);
h_Signal_Stop->GetXaxis()->SetRangeUser(-4, 4);
...
TLegend *leg1 = new TLegend(0.62,0.55,0.90,0.90,NULL,"brNDC"); 
...
h_ratio->SetTitle("; #bf{#Delta#phi(MET, trailing muon)}; Data/MC Ratio");
...
h_ratio_Unc->GetXaxis()->SetRangeUser(35.0, 300.0);
