...
int ReadLowPtSUSY_Tree(std::string infile, std::string outfile){
  ...
  TH1F *h_InvariantMass_Mu=new TH1F("h_InvariantMass_Mu", "Di-muon invariant mass; m_{#mu#mu} [GeV]; Events/GeV", 3000, 0, 300); h_InvariantMass_Mu->Sumw2();
  TH1F *h_InvariantMass_El=new TH1F("h_InvariantMass_El", "Di-electron invariant mass; m_{ee} [GeV]; Events/GeV", 3000, 0, 300); h_InvariantMass_El->Sumw2();
  TH1F *h_InvariantMass_MuPh=new TH1F("h_InvariantMass_MuPh", "Di-muon and photon invariant mass; m_{#mu#mu#gamma} [GeV]; Events/GeV", 3000, 0, 300); h_InvariantMass_MuPh->Sumw2();
  TH1F *h_InvariantMass_ElPh=new TH1F("h_InvariantMass_ElPh", "Di-electron and photon invariant mass; m_{ee#gamma} [GeV]; Events/GeV", 3000, 0, 300); h_InvariantMass_ElPh->Sumw2();
  ...
  for (int i=0; i<nEvents; ++i)
    {
     ...
     if (ph1_p4.Pt()>0.0 and photons.at(0).isTight==1 and photons.at(0).phIsoTight==1){
       ...
       //cout << "Leading photon pt (photon is tight) = " << ph1_p4.Pt() << endl;
       h_photon_eta->Fill(ph1_p4.Eta()); 
       //cout << "Leading photon eta (photon is tight) = " << ph1_p4.Eta() << endl;
       h_photon_phi->Fill(ph1_p4.Phi());
       //cout << "Leading photon phi (photon is tight) = " << ph1_p4.Phi() << endl;
       h_photon_energy->Fill(ph1_p4.E());
       //cout << "Leading photon energy (photon is tight) = " << ph1_p4.E() << endl;
    ...
    if(el1_p4.Pt()>0.0 and electrons.at(0).isTight==1 and electrons.at(0).isolation < 0.10 and leadingDeltaR > 0.3) {
      ...
      //cout << "Leading electron pt (electron is tight) = " << el1_p4.Pt() << endl;
      h_el_eta_leading->Fill(el1_p4.Eta());
      //cout << "Leading electron eta (electron is tight) = " << el1_p4.Eta() << endl;
      h_el_pt_leading->Fill(el1_p4.Pt());
      //cout << "Leading electron phi (electron is tight) = " << el1_p4.Phi() << endl;
      h_el_energy_leading->Fill(el1_p4.E());
      //cout << "Leading electron energy (electron is tight) = " << el1_p4.E() << endl;
