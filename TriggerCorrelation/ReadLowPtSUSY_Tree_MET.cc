...
int ReadLowPtSUSY_Tree_TriggerCorrelation(std::string infile, std::string outfile){
  ...
  TH2F *h_ControlTrigger1_RemoveJets = new TH2F("h_ControlTrigger1_RemoveJets", "Scatter Plot of Photon pT versus MET; Photon pT [GeV]; MET [GeV]", 1000, 0, 1000.0, 1000.0, 0, 1000.0);  h_ControlTrigger1_RemoveJets->Sumw2();
  TH2F *h_AnalysisTrigger1_RemoveJets = new TH2F("h_AnalysisTrigger1_RemoveJets", "Scatter Plot of Photon pT versus MET; Photon pT [GeV]; MET [GeV]", 1000, 0, 1000.0, 1000.0, 0, 1000.0);  h_AnalysisTrigger1_RemoveJets->Sumw2();  
  ...
  for (int i=0; i<nEvents; ++i)
   ...
   //fired_HLTMET100 corresponds to HLT_MET100_HBHENoiseCleaned_v*  

   if(photons.size() > 0){
   //Characterizing the photon leg  
   ...
   if(photons.size() > 0){
   //Characterizing the photon leg  
     if(fired_HLTMET100==1 and foundJetNearPhoton==0 and photons.at(0).pT>30.0 and photons.at(0).isTight==1 and photons.at(0).phIsoTight==1){
       h_ControlTrigger1_RemoveJets->Fill(photons.at(0).pT, MET);
       if(fired_HLTPhoIdMet==1){
         h_AnalysisTrigger1_RemoveJets->Fill(photons.at(0).pT, MET);
       }//main trigger if   
     }//control trigger if
   }
  ...
  h_ControlTrigger1_RemoveJets->Write();
  h_AnalysisTrigger1_RemoveJets->Write();
  tFile->Close();
