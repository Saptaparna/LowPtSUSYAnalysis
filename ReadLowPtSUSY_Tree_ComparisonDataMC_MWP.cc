#include "ReadLowPtSUSY_Tree_ComparisonDataMC_MWP.h"

int ReadLowPtSUSY_Tree_ComparisonDataMC_MWP(std::string infile, std::string outfile, std::string type, std::string signSelection, std::string MCSample){
...
*/
   ...
   for(unsigned int k=0; k<photons.size(); ++k)
   {
     ...
     if(fabs(photons.at(k).eta)<1.444 and photons.at(k).pT>30.0 and photons.at(k).isMedium==1 and photons.at(k).phIsoMedium==1 and photons.at(k).phpixelVeto==0)// and photons.at(k).phSigmaIetaIeta > 0.001 and photons.at(k).phSigmaIphiIphi>0.0001 and swissCross.at(k)<0.90)// and photons.at(k).phSigmaIphiIphi>0.0001)// and swissCross.at(k)<0.90)
    {
   ...
   for(unsigned int l=0; l<photons.size(); ++l)
     {
       ...
       if(fabs(photons.at(l).eta)<1.444 and photons.at(l).pT>30.0 and photons.at(l).isMedium==1 and photons.at(l).phIsoMedium==1 and photons.at(l).phpixelVeto==0){// and photons.at(l).phSigmaIetaIeta > 0.001 and photons.at(l).phSigmaIphiIphi>0.0001 and swissCross.at(l)<0.90){ //photons.at(l).phSigmaIphiIphi>0.0001 and swissCross.at(l)<0.90){
