#include "ReadLowPtSUSY_Tree_ComparisonDataMC_TWP.h"

int ReadLowPtSUSY_Tree_ComparisonDataMC_TWP(std::string infile, std::string outfile, std::string type, std::string signSelection, std::string MCSample){
...
*/
   ...
   for(unsigned int k=0; k<photons.size(); ++k)
   {
     ...
     if(fabs(photons.at(k).eta)<1.444 and photons.at(k).pT>30.0 and photons.at(k).isTight==1 and photons.at(k).phIsoTight==1 and photons.at(k).phpixelVeto==0)// and photons.at(k).phSigmaIetaIeta > 0.001 and photons.at(k).phSigmaIphiIphi>0.0001 and swissCross.at(k)<0.90)// and photons.at(k).phSigmaIphiIphi>0.0001)// and swissCross.at(k)<0.90)
    {
