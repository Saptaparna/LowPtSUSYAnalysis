...
void makeCrossSectionPlot(std::string outfile){
  ...
  double mass[6] = {100, 125, 140, 150, 175, 200};
  ...
  TGraphErrors *Xsec_mass = new TGraphErrors(6, mass, cross_section, x_error, y_error);
  Xsec_mass->SetTitle("");
  ...
  Xsec_mass->GetYaxis()->SetTitle("Cross section [fb]");
  ...
  leg1->SetFillStyle(0); 
  leg1->AddEntry(Xsec_mass,"Stop Production with a photon", "lp"); 
  leg1->Draw();
