...
void makeStackedHisto_HTb_ElMu(){
...
THStack hs("hs","HTb (scalar sum of jet pT)");

TFile* file0 = TFile::Open("MC_OS/Output_LowPtSUSY_Tree_ZGToLLG_8TeV_CaloMET_All.root");
TH1F *h0 = (TH1F*)file0->Get("h_HTb_ElMu");
h0->Rebin(50);
h0->GetXaxis()->SetRangeUser(0, 350);
...
TH1F *h1a = (TH1F*)file1a->Get("h_HTb_ElMu");
h1a->Rebin(50);
h1a->GetXaxis()->SetRangeUser(0, 350);
...
TH1F *h2 = (TH1F*)file2->Get("h_HTb_ElMu");
h2->Rebin(50);
...
hs.SetMaximum(200.0);
hs.SetMinimum(0.1);
hs.GetXaxis()->SetRangeUser(0, 350);
//hs.GetXaxis()->SetTitle("HTb (GeV)");
...
TH1F *h_Data = (TH1F*)file_Data->Get("h_HTb_ElMu");
h_Data->Rebin(50);
h_Data->SetLineColor(kBlack);
h_Data->SetMarkerStyle(20);
h_Data->SetMarkerSize(1.0);
h_Data->GetXaxis()->SetRangeUser(160, 350);
h_Data->Draw("SAME E");

TFile* file_Signal = TFile::Open("Signal/Output_LowPtSUSY_Tree_Stop100_Chargino75_Neutralino30_Ntuple_Pta20.root");
TH1F *h_Signal = (TH1F*)file_Signal->Get("h_HTb_ElMu");
h_Signal->Rebin(50);
h_Signal->GetXaxis()->SetRangeUser(0, 350);
...
TH1F *h_Signal_Stop = (TH1F*)file_Signal->Get("h_HTb_ElMu");
h_Signal_Stop->Rebin(50);
h_Signal_Stop->GetXaxis()->SetRangeUser(0, 350);
...
for (int ibin = 1; ibin < h_AllMC_Unc->GetNbinsX()+1; ibin++){
  ...
  uncSyst += //h1->GetBinContent(ibin)*h1->GetBinContent(ibin)*0.15*0.15 + 
...
h_ratio->SetTitle("; #bf{HTb (GeV)}; Data/MC Ratio");
...
gStyle->SetHatchesLineWidth(4);
gStyle->SetErrorX(0.5);
h_ratio_Unc->GetXaxis()->SetRangeUser(0, 280);
h_ratio_Unc->SetFillStyle(3005);
h_ratio_Unc->SetFillColor(1);
h_ratio_Unc->SetMarkerStyle(1);
h_ratio_Unc->Draw("SAME E2");

TBox *blot=new TBox(0, -0.5, 150, 3);
blot->SetFillColor(kWhite);
blot->Draw("same");

c1.SaveAs("h_HTb_ElMu_Stacked.pdf");
c1.SaveAs("h_HTb_ElMu_Stacked.png");
}
