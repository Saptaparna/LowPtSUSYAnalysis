...
int ReadLowPtSUSY_Tree_ControlRegion_FakePhotonTemplate(std::string infile,  std::string outfile,  std::string type){
  ...
  for (int i=0; i<nEvents ; ++i)
   ...
   for(unsigned int k=0; k<photons.size(); ++k)
   {
     ...
     if(fabs(photons.at(k).eta)<1.444 and photons.at(k).pT>30.0 and photons.at(k).phHoE < 0.05 and photons.at(0).phR9 > 0.9 and photons.at(k).phIsoTight==1 and photons.at(k).phpixelVeto==0 and swissCross.at(k)<0.90) //tight ID except sieie cut and swiss cross cut
     {
   ...
   for(unsigned int k=0; k<photons.size(); ++k)
   {
     AnalysisPhotonInfo SBPhoton; //analysis photon without sigma ietaieta

     swissCross.push_back((photons.at(k).phseedCrystalEnergy)/(photons.at(k).phe1x5 + photons.at(k).phe5x1 - photons.at(k).phseedCrystalEnergy));

     if(not(photons.at(0).chIsolation > 2.0 and photons.at(0).chIsolation < 6.0)) continue;

     if(fabs(photons.at(k).eta)<1.444 and photons.at(k).pT>30.0 and photons.at(k).phHoE < 0.05 and photons.at(0).phR9 > 0.9 and photons.at(k).phpixelVeto==0 and photons.at(0).nuIsolation < (0.4 + 0.04*photons.at(k).pT) and photons.at(0).phIsolation < (0.5 + 0.005*photons.at(k).pT) and swissCross.at(k)<0.90) //sideband region cut without sieie and swiss cross cut
     {
   ...
   if(anaPhotons.size() > 0)
     ...
     if(type=="MC"){
       if(anaPhotons.at(0).PhLV.Pt() > 30.0 and anaPhotons.at(0).PhLV.Pt() < 40.0 and anaPhotons.at(0).matched==1) h_SigmaIetaIeta_Matched_Ph30To40->Fill(anaPhotons.at(0).SigmaIetaIeta);
       if(anaPhotons.at(0).PhLV.Pt() > 40.0 and anaPhotons.at(0).PhLV.Pt() < 50.0 and anaPhotons.at(0).matched==1) h_SigmaIetaIeta_Matched_Ph40To50->Fill(anaPhotons.at(0).SigmaIetaIeta);
       if(anaPhotons.at(0).PhLV.Pt() > 50.0 and anaPhotons.at(0).PhLV.Pt() < 60.0 and anaPhotons.at(0).matched==1) h_SigmaIetaIeta_Matched_Ph50To60->Fill(anaPhotons.at(0).SigmaIetaIeta);
       if(anaPhotons.at(0).PhLV.Pt() > 60.0 and anaPhotons.at(0).PhLV.Pt() < 70.0 and anaPhotons.at(0).matched==1) h_SigmaIetaIeta_Matched_Ph60To70->Fill(anaPhotons.at(0).SigmaIetaIeta);
       if(anaPhotons.at(0).PhLV.Pt() > 70.0 and anaPhotons.at(0).PhLV.Pt() < 80.0 and anaPhotons.at(0).matched==1) h_SigmaIetaIeta_Matched_Ph70To80->Fill(anaPhotons.at(0).SigmaIetaIeta);
       if(anaPhotons.at(0).PhLV.Pt() > 80.0 and anaPhotons.at(0).PhLV.Pt() < 100.0 and anaPhotons.at(0).matched==1) h_SigmaIetaIeta_Matched_Ph80To100->Fill(anaPhotons.at(0).SigmaIetaIeta);
       if(anaPhotons.at(0).PhLV.Pt() > 100.0 and anaPhotons.at(0).PhLV.Pt() < 130.0 and anaPhotons.at(0).matched==1) h_SigmaIetaIeta_Matched_Ph100To130->Fill(anaPhotons.at(0).SigmaIetaIeta);
